...
@Component({
    ...
    <div class="catering-container">
      ...
      <table mat-table [dataSource]="orders$ | async" class="mat-elevation-z8">
        ...
        <ng-container matColumnDef="eventDate">
          ...
          <td mat-cell *matCellDef="let element"> {{element.eventDate.toDate() | date:'medium'}} </td>
        </ng-container>
  ...
  styles: [`
    ...
    .status-badge.pending { background-color: #ffcc00; color: #000; }
    .status-badge.approved { background-color: #4caf50; color: #fff; }
    .status-badge.in-progress { background-color: #2196f3; color: #fff; }
    .status-badge.completed { background-color: #9c27b0; color: #fff; }
  `]
})
export class CateringManagementComponent implements OnInit {
  displayedColumns: string[] = ['clientName', 'eventDate', 'eventType', 'status', 'actions'];
  orders$: Observable<CateringOrder[]>;
  ...
  ngOnInit() {
    this.orders$ = this.cateringService.getCateringOrders();
  }
  ...
  openDialog(order?: CateringOrder) {
    ...
    dialogRef.afterClosed().subscribe(result => {
      if (result) {
        if (order) {
          this.cateringService.updateCateringOrder({ ...order, ...result }).then(() => {
            this.snackBar.open('Order updated', 'Close', { duration: 3000 });
          });
        } else {
          this.cateringService.addCateringOrder(result).then(() => {
            this.snackBar.open('Order added', 'Close', { duration: 3000 });
          });
        }
      }
    });
  ...
  deleteOrder(id: string) {
    if (confirm('Are you sure you want to delete this order?')) {
      this.cateringService.deleteCateringOrder(id).then(() => {
...
@Component({
  selector: 'app-catering-order-dialog',
  standalone: true,
  imports: [
    CommonModule,
    ReactiveFormsModule,
  ...
  template: `
    <h2 mat-dialog-title>{{data.id ? 'Edit' : 'Add'}} Catering Order</h2>
