...
@Component({
  ...
  standalone: true,
  imports: [CommonModule, MatGridListModule, MatButtonModule, MatIconModule, MatDialogModule, MatInputModule, MatFormFieldModule, MatSelectModule, MatSnackBarModule, ReactiveFormsModule],
  template: `<div class="gallery-container"><div class="header"><h1>Gallery Management</h1><button mat-raised-button color="primary" (click)="openDialog()"><mat-icon>add_photo_alternate</mat-icon> Upload Image</button></div><mat-grid-list cols="3" rowHeight="1:1" gutterSize="10px"><mat-grid-tile *ngFor="let image of images$ | async"><img [src]="image.imageUrl" [alt]="image.title"><mat-grid-tile-footer><h3>{{image.title}}</h3><span class="spacer"></span><button mat-icon-button (click)="deleteImage(image.id)"><mat-icon>delete</mat-icon></button></mat-grid-tile-footer></mat-grid-tile></mat-grid-list></div>`,
  styles: [`.gallery-container{padding:20px}.header{display:flex;justify-content:space-between;align-items:center;margin-bottom:20px}img{width:100%;height:100%;object-fit:cover}.spacer{flex:1 1 auto}`]
})
export class GalleryManagementComponent implements OnInit {
  ...
  async openDialog() { const ref = this.dialog.open(GalleryImageDialogComponent, { width: '400px' }); const result = await ref.afterClosed().toPromise(); if (result) { await this.galleryService.addGalleryImage({ ...result, uploadedAt: new Date() as any }).toPromise(); this.snackBar.open('Image uploaded', 'Close', { duration: 3000 }); } }
  async deleteImage(id?: string) { if (id && confirm('Are you sure you want to delete this image?')) { await this.galleryService.deleteGalleryImage(id); this.snackBar.open('Image deleted', 'Close', { duration: 3000 }); } }
}
