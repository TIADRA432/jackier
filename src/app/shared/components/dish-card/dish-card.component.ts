    ...
    <div class="bg-white rounded-3xl shadow-lg overflow-hidden hover:shadow-2xl transition-all duration-500 group h-full flex flex-col border border-gray-100 transform hover:-translate-y-1">
      <div class="relative h-64 overflow-hidden bg-gray-100">
        ...
        <div class="absolute top-4 right-4 flex flex-col gap-2 items-end">
          @if (dish().isFeatured) { <span class="rounded-full bg-jacquier-primary px-3 py-2 text-xs font-bold text-white">Suggestion de la Cheffe</span> }
          @if (dish().isSpicy) { <span class="rounded-full bg-white px-3 py-2 text-xs font-bold text-red-700">Épicé</span> }
